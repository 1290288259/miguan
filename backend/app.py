...
# 从 extensions 模块导入全局唯一的 socketio 实例
# 这样确保所有模块(log_service等)导入的都是同一个实例，
# 不受 werkzeug reloader 重新加载 app.py 的影响
...
def create_app():
    ...
    # 核心：初始化全局的 socketio 配置跨域
    socketio.init_app(app, cors_allowed_origins="*", async_mode="threading")

    # 注册 /ws 命名空间的连接/断开事件处理器
    # 必须注册至少一个 handler，否则客户端无法连接到该 namespace
    @socketio.on("connect", namespace="/ws")
    def handle_ws_connect():
        """客户端连接到 /ws 命名空间时触发"""
        print(f"[WebSocket] 客户端已连接到 /ws 命名空间, sid={request.sid}")

    @socketio.on("disconnect", namespace="/ws")
    def handle_ws_disconnect():
        """客户端从 /ws 命名空间断开时触发"""
        print(f"[WebSocket] 客户端已断开 /ws 命名空间, sid={request.sid}")
...
if __name__ == "__main__":
    ...
    # 核心：使用 socketio.run 代替纯粹的 app.run 这个可以正确接收 websocket 连接
    socketio.run(
        app,
        debug=True,
        use_reloader=True,
